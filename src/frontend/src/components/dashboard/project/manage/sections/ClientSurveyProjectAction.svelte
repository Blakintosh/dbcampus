<script lang="ts">
    ...
    async function getClientSurveyQuestions() {
		const res = await fetch("/api/dashboard/getSurveyQuestions?type=client");
		
		if(!res.ok) {
			throw new Error("Failed to fetch client survey questions");
		}
...
<div>
    ...
    <ul class="my-2 border-t border-b border-slate-600 max-h-[28rem] overflow-auto">
        {#await promise}
